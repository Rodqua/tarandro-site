...
function providerBadge(provider: string) {
  ...
  if (provider === 'outlook') return { label: 'Outlook', color: 'bg-blue-100 text-blue-700' }
...
function providerUrl(thread: EmailThread) {
  ...
  if (thread.account.provider === 'outlook') return `https://outlook.live.com/mail/0/`
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-50 overflow-hidden">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {/* Liste emails */}
        <main className={`flex-1 overflow-y-auto bg-white border-r border-gray-200 ${selected ? 'hidden sm:block sm:w-80 sm:flex-none' : 'w-full'}`}>
          {loading ? (
              ...
              {filtered.map(thread => {
                ...
                return (
                  <div
                    ...
                    className={`flex items-start gap-2.5 px-4 py-3 cursor-pointer transition-colors group ${isSelected ? 'bg-indigo-50 border-l-2 border-indigo-500' : thread.isUnread ? 'hover:bg-gray-50 bg-blue-50/30' : 'hover:bg-gray-50'}`}
                  >
            ...
            <div className="px-6 py-4 border-b border-gray-200 flex-shrink-0">
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  <a href={providerUrl(selected)} target="_blank" rel="noopener noreferrer" className="px-3 py-1.5 text-xs bg-gray-100 hover:bg-gray-200 rounded-lg text-gray-600 transition-colors" title="Ouvrir dans le client mail">
                    ↗ Ouvrir
                  </a>
                  <button
              ...
              {!showReply ? (
                ...
                <div className="px-6 py-4">
                  <div className="text-xs text-gray-500 mb-2 font-medium">
                    Réponse à : {extractEmail(selected.sender)}
                  </div>
                  <textarea
                    value={replyBody}
                  ...
                  <div className="flex gap-2 mt-2">
                    ...
                    <button onClick={() => { setShowReply(false); setReplyBody('') }} className="px-4 py-2 text-gray-600 hover:bg-gray-100 rounded-lg text-sm transition-colors">
                      Annuler
                    </button>
                  </div>
                </div>
